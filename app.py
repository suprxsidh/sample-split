...
def simplify_debts(balances, _members):
    creditors = []
    debtors = []
...
def remove_member(group_id, user_id):
    ...
    removed_user = db.session.get(User, user_id)
    db.session.delete(membership)
    db.session.commit()
...
def manage_recurring(group_id):
    ...
    if request.method == "POST":
        ...
        if action == "create":
            ...
            if not amount or amount <= 0:
                ...
                return render_template(
                    "recurring.html", group=group_obj, members=members, recurring=group_obj.recurring_expenses
                )
            ...
            for member in members:
                split = ExpenseSplit(
                    expense_id=expense.id, user_id=member.id, amount_owed=round(recurring.amount / len(members), 2)
                )
                db.session.add(split)

            recurring.last_created = datetime.now(timezone.utc)
            db.session.commit()
    ...
    return render_template(
        "recurring.html", group=group_obj, members=members, recurring=recurring, categories=group_obj.categories
    )
...
def api_simplify(group_id):
    ...
    result = []
    for t in simplified:
        from_user = db.session.get(User, t["from"])
        to_user = db.session.get(User, t["to"])
        result.append(
            {
...
def export_group(group_id):
    ...
    return (
        ...
        200,
        {
            ...
            "Content-Disposition": f"attachment; filename={group_obj.name.replace(' ', '_')}_report.pdf",
        },
    )
...
def group_summary(group_id):
    ...
    for cat in categories:
        ...
        if cat_total > 0:
            category_breakdown[cat.id] = {
                "name": cat.name,
                "color": cat.color,
                "total": cat_total,
                "percentage": (cat_total / total_spent * 100) if total_spent > 0 else 0,
            }
        ...
        category_status.append(
            {
                ...
                "remaining": (cat.budget_limit - spent) if cat.budget_limit else None,
                "over": (spent > cat.budget_limit) if cat.budget_limit else False,
            }
        )
...
def admin_password_reset_approve(reset_id):
    ...
    reset_request = db.get_or_404(PasswordReset, reset_id)
...
def seed_database():
    ...
    diana = User(username="diana", email="diana@test.com")
    diana.set_password("testpass123")

    db.session.add_all([alice, bob, charlie, diana])
    db.session.flush()

    group1 = Group(name="Weekend Trip", invite_code=Group.generate_invite_code(), created_by_id=alice.id)
    group2 = Group(name="Roommates", invite_code=Group.generate_invite_code(), created_by_id=bob.id)
    group3 = Group(name="Dinner Club", invite_code=Group.generate_invite_code(), created_by_id=charlie.id)
    db.session.add_all([group1, group2, group3])
    db.session.flush()

    cat1_food = Category(group_id=group1.id, name="Food", icon="utensils", color="#ef4444")
    cat1_transport = Category(group_id=group1.id, name="Transport", icon="car", color="#3b82f6")
    cat1_entertainment = Category(group_id=group1.id, name="Entertainment", icon="film", color="#a855f7")
    db.session.add_all([cat1_food, cat1_transport, cat1_entertainment])

    cat2_groceries = Category(group_id=group2.id, name="Groceries", icon="cart", color="#22c55e")
    cat2_utilities = Category(group_id=group2.id, name="Utilities", icon="lightning", color="#f59e0b")
    db.session.add_all([cat2_groceries, cat2_utilities])

    cat3_food = Category(group_id=group3.id, name="Food", icon="utensils", color="#ef4444")
    cat3_drinks = Category(group_id=group3.id, name="Drinks", icon="wine", color="#ec4899")
    db.session.add_all([cat3_food, cat3_drinks])
    db.session.flush()

    db.session.add(GroupMember(user_id=alice.id, group_id=group1.id))
    db.session.add(GroupMember(user_id=bob.id, group_id=group1.id))
    db.session.add(GroupMember(user_id=charlie.id, group_id=group1.id))

    db.session.add(GroupMember(user_id=bob.id, group_id=group2.id))
    db.session.add(GroupMember(user_id=charlie.id, group_id=group2.id))
    db.session.add(GroupMember(user_id=diana.id, group_id=group2.id))

    db.session.add(GroupMember(user_id=alice.id, group_id=group3.id))
    db.session.add(GroupMember(user_id=bob.id, group_id=group3.id))
    db.session.add(GroupMember(user_id=charlie.id, group_id=group3.id))
    db.session.add(GroupMember(user_id=diana.id, group_id=group3.id))
    db.session.flush()
    ...
    db.session.add(exp1)
    db.session.flush()
    db.session.add(ExpenseSplit(expense_id=exp1.id, user_id=alice.id, amount_owed=200))
    db.session.add(ExpenseSplit(expense_id=exp1.id, user_id=bob.id, amount_owed=200))
    ...
    db.session.add(exp2)
    db.session.flush()
    db.session.add(ExpenseSplit(expense_id=exp2.id, user_id=alice.id, amount_owed=100))
    db.session.add(ExpenseSplit(expense_id=exp2.id, user_id=bob.id, amount_owed=100))
    ...
    db.session.add(exp3)
    db.session.flush()
    db.session.add(ExpenseSplit(expense_id=exp3.id, user_id=alice.id, amount_owed=150))
    db.session.add(ExpenseSplit(expense_id=exp3.id, user_id=bob.id, amount_owed=150))
    ...
    db.session.add(exp4)
    db.session.flush()
    db.session.add(ExpenseSplit(expense_id=exp4.id, user_id=bob.id, amount_owed=133.33))
    db.session.add(ExpenseSplit(expense_id=exp4.id, user_id=charlie.id, amount_owed=133.33))
    ...
    db.session.add(exp5)
    db.session.flush()
    db.session.add(ExpenseSplit(expense_id=exp5.id, user_id=alice.id, amount_owed=100))
    db.session.add(ExpenseSplit(expense_id=exp5.id, user_id=bob.id, amount_owed=100))
    db.session.add(ExpenseSplit(expense_id=exp5.id, user_id=charlie.id, amount_owed=100))
    db.session.add(ExpenseSplit(expense_id=exp5.id, user_id=diana.id, amount_owed=100))
...
def seed():
    """Create test users and groups for development."""
    seed_database()
